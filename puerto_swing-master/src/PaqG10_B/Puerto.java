...
public class Puerto {
    ...
    public String toString(){
        ...
        for(int i=0;i<F;i++){
            for(int j=0;j<C;j++){
                ...
                else s.append(almacen[i][j].getId()+"-"+almacen[i][j].getPeso()+"    ");
            }
            s.append("\n");
        }
        return s.toString();
    ...
    public int sumarPesos(){
        int sumatotal=0;
        for(int i=0;i<F;i++) {       // bucle para recorrer el almacen entero
            for (int j = 0;j<C; i++) {
                if(this.almacen[i][j].getId() != -1){
                    sumatotal = sumatotal + this.almacen[i][j].getPeso();
                }
            }
        }
       return sumatotal;
